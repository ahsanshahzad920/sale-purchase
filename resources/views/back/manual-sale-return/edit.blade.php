    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Edit Manual Return</h3>
            </div>
                ...
                <div class="card card-shadow rounded-3 border-0 mt-5">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="exampleFormControlSelect1" class="mb-1 fw-bold">Reference <span
                                            class="text-danger">*</span></label>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table text-center mt-2" id="mainTable">
                            ...
                            <tbody>
                                ...
                                @endphp --}}
                                    @php
                                        $warehouse_product = \App\Models\ProductWarehouse::where('product_id',$product->product->id)->where('warehouse_id',$sale_return->warehouse_id)->first();
                                        ...
                                        if($product->product->product_type != 'service'){
                                            if($product->product->product_unit != $product->purchase_units?->id){
                                                if($product->product->unit->parent_id == 0){
                                                    $expression = $warehouse_product->quantity . $product->product->unit->operator . $product->return_units?->operator_value;
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle"> <span class="badges bg-darkwarning p-1">{{ $quantity ?? '' }} {{ $product->return_units?->short_name ? $product->return_units?->short_name : ''}}</span></td>
                                        ...
                                        <td class="align-middle">{{ $sale_return->discount ?? '' }} </td>
                                        <td class="align-middle">{{ $sale_return->order_tax ?? '' }} </td>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#createSaleForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/manual-sale-return/{{$sale_return->id}}',
                    type: 'PUT',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        window.location.href = "{{ route('manual-sale-return.index') }}";
