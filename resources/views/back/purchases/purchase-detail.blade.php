    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Purchase Details</h3>
            </div>
            ...
            <div class="card card-shadow rounded-3 border-0 mt-4 p-3">
                <div class="card-body">
                    <div class="mb-4">
                        {{-- <a href="#" class="btn create-btn rounded-3 mt-2"
                      >Filter <i class="bi bi-funnel"></i
                    ></a>
                    <a href="#" class="btn rounded-3 mt-2 excel-btn"
                    ...
                    <div id="invoice">
                        <div class="row mb-4">
                            <div class="col-md-4 col-6 mt-2">
                                <h5 class="text-start mt-2 pb-1 heading">Supplier Info</h5>
                                <p class="m-0">{{ $purchase->vendor->user->name ?? '' }}</p>
                                <p class="m-0">{{ $purchase->vendor->user->email ?? '' }}</p>
                                <p class="m-0">{{ $purchase->vendor->user->contact_no ?? '' }}</p>
                                <p class="m-0">{{ $purchase->vendor->user->address ?? '' }}</p>
                            </div>
                            <div class="col-md-4 col-6 mt-2">
                                <h5 class="text-start mt-2 pb-1 heading">Company Info</h5>
                                <p class="m-0">{{ $purchase->invoice->user->name ?? '' }}</p>
                                <p class="m-0">{{ $purchase->invoice->user->email ?? '' }}</p>
                                <p class="m-0">{{ $purchase->invoice->user->contact_no ?? '' }}</p>
                                <p class="m-0">{{ $purchase->invoice->user->address ?? '' }}</p>
                            </div>
                            <div class="col-md-4 col-6 mt-2">
                                <h5 class="text-start mt-2 pb-1 heading">Purchase Info</h5>
                                <p class="m-0">Reference : {{ $purchase->reference ?? '' }}</p>
                                <p class="m-0">Payment Status : <span
                                        class="badges green-border text-center">{{ $purchase->payment_status ?? '' }}</span>
                                </p>
                                <p class="m-0">Warehouse : {{ $purchase->warehouse->users->name ?? '' }}</p>
                                <p class="m-0">Status : <span
                                        class="badges blue-border text-center">{{ $purchase->status ?? '' }}</span></p>
                            </div>
                        </div>
                                    ...
                                    @foreach ($purchase->purchaseItems as $product)
                                        <tr>
                                            ...
                                            <td class="align-middle"> {{ $product->quantity ?? '' }}
                                                {{ $product->purchase_units->short_name ?? '' }}</td>
                        ...
                        <div class="row mt-2 px-3 ">
                            ...
                            <div class="col-md-4 border rounded-2 py-2">
                                ...
                                <div class="row border-bottom">
                                    ...
                                    <div class="col-md-6 col-6">$ {{ $purchase->shipping ?? '0.00' }}</div>
                                </div>

                                <div class="row border-bottom">
                                    <div class="col-md-6 col-6">Grand Total</div>
                                    <div class="col-md-6 col-6">$ {{ $purchase->grand_total ?? '0.00' }}</div>
                                </div>
                                <div class="row border-bottom">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 col-6">$ {{ $purchase->amount_due ?? '0.00' }}</div>
                                </div>
                            </div>
                        </div>
