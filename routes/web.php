...
use App\Http\Controllers\SuperAdmin\Auth\AuthController as SuperAdminAuthController;
...
Route::domain(env('APP_URL'))->group(function () {
    // Auth::routes();
    Route::get('/', [TenantController::class, 'site'])->name('site');

    Route::get('/update-tenant', [TenantController::class, 'updateTenant']);

    Route::prefix('auth')->group(function () {
        Route::get('login', [SuperAdminAuthController::class,'login'])->name('auth.login');
        Route::post('login', [SuperAdminAuthController::class, 'loginPost'])->name('auth.login.post');
        Route::get('register', [SuperAdminAuthController::class, 'register'])->name('auth.register');
        Route::post('register', [SuperAdminAuthController::class, 'registerPost'])->name('auth.register.post');
        Route::post('logout', [SuperAdminAuthController::class, 'logout'])->name('auth.logout');
        Route::get('/verify-domain/{subdomain}/{token}', [SuperAdminAuthController::class, 'verifyDomain'])->name('verification.verify');
    });
    Route::prefix('administrator')->as('super.')->group(function () {
        ...
        Route::get('transactions', [\App\Http\Controllers\SuperAdmin\SubscriptionController::class,'transactions'])->name('transactions.index');
        Route::put('customers/{tenantId}/status', [TenantController::class, 'updateStatus'])->name('customers.status.update');
        Route::get('contact',[SuperAdminDashboardController::class,'contact'])->name('contact.index');
        Route::delete('contact/{id}',[SuperAdminDashboardController::class,'contactDelete'])->name('contact.destroy');
        Route::get('setting',[SuperAdminDashboardController::class,'setting'])->name('setting.index');
        Route::post('setting/store',[SuperAdminDashboardController::class,'settingStore'])->name('setting.store');
        Route::post('add/plans',[\App\Http\Controllers\SuperAdmin\CustomerController::class,'addPlanToCustomers'])->name('add.plans');
        Route::resource('coupons',CouponController::class);

    });
    Route::post('upgrade-plan', [SuperAdminDashboardController::class, 'purchasePlan'])->name('purchasePlan');

    Route::get('upgrade-plan/success', [SuperAdminDashboardController::class, 'upgradePlanSuccess'])->name('upgrade-plan.success');
    Route::get('upgrade-plan/cancel', [SuperAdminDashboardController::class, 'upgradePlanCancel'])->name('upgrade-plan.cancel');

    Route::post('contact/store',[TenantController::class,'contactStore'])->name('contact.store');

    Route::get('custom-domain/requests',[TenantController::class,'customDomainRequests'])->name('custom-domain.requests');
    // approved
    Route::put('custom-domain/requests/{id}/approved', [TenantController::class, 'approveCustomDomain'])->name('custom-domain.requests.approved');
    // rejected
    Route::put('custom-domain/requests/{id}/rejected', [TenantController::class, 'rejectCustomDomain'])->name('custom-domain.requests.rejected');
    // delete
    Route::delete('custom-domain/requests/{id}', [TenantController::class, 'deleteCustomDomainRequest'])->name('custom-domain.requests.delete');

    Route::get('contact-us',[TenantController::class,'contactUs'])->name('contacts');
    Route::post('contact-us/store',[TenantController::class,'contactUsStore'])->name('contacts.store');
...
Route::domain('{subdomain}.'.env('APP_URL'))->middleware(['append.subdomain','check.subdomain.status','check.plan.limitation'])->group(function () {
    ...
    // });
    Route::get('/category/{code}/product/{sku}', [UserProductController::class, 'index'])->name('user.product.index');

    // Route::get('/category/{code}/product/{sku}', function($code, $sku) {
    //     return "Raw route works: $code - $sku";
	...
	*/
	    ...
	    */
            ...
            Route::put('category/status/{category}', [CategoryController::class, 'change_status'])->name('category.change_status');
